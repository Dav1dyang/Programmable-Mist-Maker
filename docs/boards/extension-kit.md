...
The simplest way to drive mist from a
[Seeed XIAO ESP32-C6](https://www.seeedstudio.com/Seeed-Studio-XIAO-ESP32C6-Pre-Soldered-p-6328.html):
an expansion PCB that mounts directly on the XIAO. USB-C powered, no battery, no boost
converter — just the piezo drive stage and an INA180 current-sense amp so your code can
*feel* the disc working.
...
[Full BOM on GitHub](https://github.com/Dav1dyang/Programmable-Mist-Maker/blob/main/variants/xiao-extension-kit/hardware/production/bom.csv)
...
1. Order PCBs with the JLCPCB production files in
   ...
   (SMT assembly recommended — everything except the connectors can be machine-placed).
...
- Power budget: a misting disc draws roughly 0.3–0.5 A from the 5 V USB rail — any
  normal USB port or charger is fine.
- Safety, cleaning, and known-issues notes live in [How It Works](../how-it-works.md).
